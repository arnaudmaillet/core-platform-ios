...
struct PagedTabBarPillDragTests {
    ...
    // MARK: - Telling a host that has no pager

    /// ⚠️ **THE DRAG IS OTHERWISE UNOBSERVABLE TO SUCH A HOST.** `setProgress`
    /// moves `selectedIndex` deliberately without announcing, so a bar with no
    /// pager lands the pill on the right segment and tells nobody. Reported from a
    /// device on Upload's editor: sliding onto a category left the band showing
    /// the previous one until the viewer also tapped it.
    ///
    /// ⚠️ THE LANDING IS COMPARED TO THE BAR'S OWN `selectedIndex`, not to an
    /// index written here. `landingIndex` adds half a second of throw per unit of
    /// velocity, so pinning a number would be pinning my arithmetic rather than
    /// the behaviour — and would fail for the wrong reason the day that throw is
    /// tuned.
    @Test func aSettledDragTellsAHostWithNoPager() {
        let bar = bar()
        var landed: Int?
        bar.onSettled = { landed = $0 }

        let start = pillCentre(bar)
        #expect(bar.debugBeginPillDrag(atViewportX: start))
        // Several damped steps: one 120pt jump in 1/60s reports a flick, and a
        // flick's throw carries the landing somewhere this test cannot predict.
        for step in 1...4 {
            bar.debugDragPill(toViewportX: start + CGFloat(step) * 30, after: 0.05)
        }
        bar.debugEndPillDrag()

        #expect(landed != nil, "a settle with no pager must say where it landed")
        #expect(landed == bar.selectedIndex, "and agree with the bar: \(landed as Any) vs \(bar.selectedIndex)")
        #expect(landed != 0, "witness: it really moved off the segment it started on")
    }

    /// ⚠️ **THE WITNESS FOR THE GUARD.** Without this, "a settle announces" is
    /// equally true of a bar that announces EVERY settle — including a drag that
    /// wandered off a segment and came back, which changed nothing and must say
    /// nothing.
    @Test func aDragThatComesHomeAnnouncesNothing() {
        let bar = bar()
        var announcements = 0
        bar.onSettled = { _ in announcements += 1 }

        let start = pillCentre(bar)
        #expect(bar.debugBeginPillDrag(atViewportX: start))
        bar.debugDragPill(toViewportX: start + 40, after: 0.05)
        // Home again, slowly, so the release carries no throw of its own.
        bar.debugDragPill(toViewportX: start, after: 0.5)
        bar.debugEndPillDrag()

        #expect(announcements == 0, "nothing changed, so nothing is announced")
        #expect(bar.selectedIndex == 0, "and the bar is where it began")
    }

    /// ⚠️ **THE TWO CHANNELS ARE EXCLUSIVE, AND SIX SHIPPING SCREENS DEPEND ON
    /// IT.** Messages, Profile, Profile-relationships, Search, For You,
    /// PlaceProfile and the media picker all wire `onScrubEnd` and learn the
    /// landing from their pager. A bar that also announced would commit their
    /// model twice, to two answers, in an order nobody chose — which is the thing
    /// `PagedTabBar`'s own note forbids.
    @Test func aHostWithAPagerNeverHearsTheLanding() {
        let bar = bar()
        var settled = 0
        var releases = 0
        bar.onScrubEnd = { _ in releases += 1 }
        bar.onSettled = { _ in settled += 1 }
        ...
        #expect(releases == 1, "guard: the pager path really did run, so the silence below means something")
        #expect(settled == 0, "the pager answers for this host; the bar must not")
    }
}
