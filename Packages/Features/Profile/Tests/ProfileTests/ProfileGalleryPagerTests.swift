...
///
/// ⚠️ Restored with the drag it belongs to (it was deleted with the first one,
/// in 6f8b6a0). The arithmetic is unchanged, because what was wrong with that
/// gesture was never this half of it.
///
/// The selector's selection pill can be picked up and dragged, which writes the
/// pager's offset directly, frame by frame — so the scroll view never decelerates and
