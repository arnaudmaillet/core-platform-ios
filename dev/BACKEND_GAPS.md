...
## 23. Video publishes as an image asset — no video kind, no `asset_id`, and a synchronous resolve

**Status: client-side unblocked 2026-09-15; the fleet cannot serve it.** The
Upload flow now publishes a picked video end to end — `MediaLibraryReading`
vends a file URL, `VideoExporter` normalizes it to MP4, and `PostComposer`
uploads it and a poster still. Against `MockBFF` this works completely. Against
the real fleet, three contract facts make it a mock-only feature.

**1. `media.v1.MediaKind` has no video value.** `IssueUploadTicket` is sent with
`kind = .postImage` and `declared_mime_type = "video/mp4"`. The mock ignores
`kind` and routes on the mime, which is why it passes here. A fleet that
validates the pair will reject it, and one that does not will file the asset
under the wrong kind.
→ **Needs** `MEDIA_KIND_POST_VIDEO`, already drafted in
`PHASE3_VIDEO_BACKEND.md` §2.

**2. `post.v1.MediaAttachmentInput` has no `asset_id`** — only `cdn_url`. So a
post can reference a *resolved URL* and nothing else, and for video that URL does
not exist at publish time.
→ **Needs** `asset_id` on the input, with the read side resolving delivery.
Recorded in `IOS_VIDEO_CAPTURE_UPLOAD.md` §1 and §4.

**3. `resolveDeliveryURL` polls 6×1s and then throws.** That budget was written
for an image. A video transcode is seconds to minutes, so the poll will time out
and the publish will fail *after* the bytes have already been uploaded —
the worst shape of failure available. The client cannot fix this without (2):
with an `asset_id` there is nothing to wait for.

**Also unresolved, and it is ours to notice:** the poster we upload is the
client's own first frame, because `MEDIA_RENDITION_KIND_POSTER` (§7 of the Phase
3 spec) is generated by a worker that does not run (#2). When it does, the server
poster should win — a client frame-zero is a stand-in, not the contract.
